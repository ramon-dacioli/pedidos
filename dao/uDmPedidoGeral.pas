...
begin
  ...
  try
    FDQuery.Connection := DmConexao.sqlConexao; // Define a conexão FireDAC
    FDQuery.SQL.Text := 'SELECT COALESCE(MAX(numero_pedido), 0) + 1 AS SEQ FROM pedidos_dados_gerais'; // Consulta SQL para gerar o próximo número de pedido
    FDQuery.Open;

    Result := FDQuery.FieldByName('SEQ').AsInteger; // Obtém o valor da sequência
  finally
